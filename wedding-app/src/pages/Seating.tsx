...
export default function Seating() {
  ...
  const { rows: tables, insert, update: updateTable, remove: removeTable } = useTable<Table>('tables')
  ...
  // Only RSVP'd banquet guests are eligible for seating (toggle to show all).
  ...
  return (
    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragStart={onDragStart} onDragEnd={onDragEnd}>
      <PageHeader
        ...
        action={
          <Btn onClick={() => insert({ label: `Table ${tables.length + 1}`, capacity: 10, sort: tables.length } as Partial<Table>)}>
            <Plus className="w-4 h-4 inline -mt-0.5 mr-1" />Add table
          </Btn>
        }
      />
        ...
        <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
          {tables.map((t) => (
            <TableCard
              ...
              onCapacity={(capacity) => updateTable(t.id, { capacity })}
              onRemove={() => {
                byTable(t.id).forEach((g) => update(g.id, { table_id: null }))
                removeTable(t.id)
              }}
            />
          ))}
...
function TableCard({
  table, guests, onRename, onCapacity, onRemove,
}: {
  table: Table; guests: Guest[]
  onRename: (v: string) => void; onCapacity: (v: number) => void; onRemove: () => void
}) {
    ...
    <Card className={`p-3 transition ${isOver ? 'ring-2 ring-blush-400 bg-blush-50/50' : ''}`}>
      <div className="flex items-center justify-between mb-2">
        <input
          ...
          onBlur={(e) => e.target.value !== table.label && onRename(e.target.value)}
          className="font-display text-lg text-blush-700 bg-transparent outline-none w-28 border-b border-transparent focus:border-blush-300"
        />
        <div className="flex items-center gap-1.5">
          <span className={`text-xs font-semibold flex items-center gap-1 px-2 py-0.5 rounded-full ${over ? 'bg-red-100 text-red-600' : 'bg-sage-100 text-sage-700'}`}>
            ...
            <input
              ...
              onBlur={(e) => Number(e.target.value) !== table.capacity && onCapacity(Number(e.target.value))}
              className="w-6 bg-transparent outline-none text-center"
            />
          </span>
          <button onClick={onRemove} className="p-1 rounded hover:bg-red-50 text-gray-300 hover:text-red-500"><Trash2 className="w-3.5 h-3.5" /></button>
        </div>
      </div>
